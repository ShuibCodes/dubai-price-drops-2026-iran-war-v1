...
function agentKbFilesLoaded(agent) {
  const n = agent?.kbFilesLoaded;
      ...
      setAgents(
        Array.isArray(data.agents)
          ? data.agents.map((a) => {
              ...
              const kbRaw = base.kbFilesLoaded;
              const kbLoaded =
                typeof kbRaw === "number" && Number.isFinite(kbRaw) ? kbRaw : 0;
              const { kbFilesLoaded: _ignored, ...rest } = base;
              return {
                ...rest,
                kbFilesLoaded: kbLoaded,
                role: rest.role === "admin" ? "admin" : "agent",
              };
            })
          : []
      );
      ...
      const res = await fetch("/api/onboard/save-roster", {
        ...
        body: JSON.stringify({
          ...
          agents: agents.map((a) => {
            const row =
              a && typeof a === "object" && !Array.isArray(a) ? { ...a } : {};
            delete row.kbFilesLoaded;
            return {
              id: row.id,
              name: row.name ?? "",
              whatsapp: row.whatsapp ?? "",
              role: row.role ?? "agent",
              gmailToken: row.gmailToken ?? null,
              gmailEmail: row.gmailEmail ?? null,
              kbFiles: Array.isArray(row.kbFiles) ? row.kbFiles : [],
              lastGmailSync: row.lastGmailSync ?? null,
            };
          }),
        }),
      });
  ...
  return (
    <>
      ...
      <section className="mb-8 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm">
        ...
        {loading ? (
          <p className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-8 text-center text-sm text-[var(--muted)]">
            Loading roster…
          </p>
        ) : (
