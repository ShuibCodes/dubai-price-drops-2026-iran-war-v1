...
check(
  ...
  noPage.status >= 300 && noPage.status < 400 && noPageTo.includes("/copilot?next="),
  `${noPage.status} -> ${noPageTo}`
);

// The old login path survives as a stub; the middleware bounces it home.
const oldLogin = await hit("/copilot/login?next=/copilot/1416");
const oldLoginTo = oldLogin.headers.get("location") || "";
check(
  "legacy /copilot/login redirects to /copilot",
  oldLogin.status >= 300 &&
    oldLogin.status < 400 &&
    oldLoginTo.includes("/copilot?next=") &&
    !oldLoginTo.includes("/copilot/login"),
  `${oldLogin.status} -> ${oldLoginTo}`
);
